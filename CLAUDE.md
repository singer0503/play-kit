...
**所有 scalable pixel 值都用 `calc(N * var(--pk-px))` 表達**。

`--pk-px` 是定義在 `.pk-game` 上的 helper variable：
```css
.pk-game {
  --pk-px: calc(1px * var(--pk-scale, 1));
}
```
- 預設（未掛 hook 時）：`--pk-scale` fallback = 1 → `--pk-px` = 1px → 一切照設計尺寸渲染
- Hook 啟動後：`--pk-scale` 變 0..1 → `--pk-px` 變 0..1px → 所有用 `calc(N * var(--pk-px))` 的值等比縮

| 類別 | 改寫前 ❌ | 改寫後 ✅ |
|---|---|---|
| 寬高 | `width: 360px` | `width: calc(360 * var(--pk-px))` |
| Padding / margin | `padding: 20px` | `padding: calc(20 * var(--pk-px))` |
| Border-radius | `border-radius: 12px` | `border-radius: calc(12 * var(--pk-px))` |
| Font-size | `font-size: 14px` | `font-size: calc(14 * var(--pk-px))` |
| Box-shadow blur | `box-shadow: 0 4px 12px ...` | `box-shadow: 0 calc(4 * var(--pk-px)) calc(12 * var(--pk-px)) ...` |
| Transform translate | `transform: translate(14px, -18px)` | `transform: translate(calc(14 * var(--pk-px)), calc(-18 * var(--pk-px)))` |
| Keyframe 位移 | `50% { transform: translateY(-180px); }` | `50% { transform: translateY(calc(-180 * var(--pk-px))); }` |
| Canvas 尺寸（JS） | `canvas.width = 320 * dpr` | `canvas.width = 320 * scale * dpr`（scale 從 element.style.getPropertyValue 讀）|

**例外（保持 raw px、不 scale）**：
- `border-width: 1px` 等視覺粒度（< 2px 縮放會看不見）
- `outline: 2px` focus ring（a11y 要求恆定可視）
- 線性漸層 stops 的 `%`（本身就是比例）
- `rgba()` / `oklch()` 顏色
