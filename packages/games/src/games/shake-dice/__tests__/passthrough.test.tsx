...
describe('ShakeDice — HTML passthrough', () => {
  beforeEach(() => stubMatchMedia(false));

  it('className merge', () => {
    const { container } = render(<ShakeDice className="custom-host" />, { wrapper: Wrapper });
  ...
  it('id / style / data-* / aria-*', () => {
    ...
    const root = container.querySelector('section');
    expect(root?.id).toBe('my-sd');
    expect(root?.getAttribute('style')).toMatch(/max-width:\s*400px/);
    expect(root?.getAttribute('data-testid')).toBe('sd-root');
    expect(root?.getAttribute('aria-describedby')).toBe('hint');
  });

  it('role / tabIndex / lang / dir', () => {
    const { container } = render(<ShakeDice role="region" tabIndex={-1} lang="zh-TW" dir="ltr" />, {
      wrapper: Wrapper,
    });
    const root = container.querySelector('section');
