...
describe('RingToss — HTML passthrough', () => {
  beforeEach(() => stubMatchMedia(true));

  it('className merge', () => {
    const { container } = render(<RingToss className="custom-host" />, { wrapper: Wrapper });
  ...
  it('id / style / data-* / aria-*', () => {
    ...
    const root = container.querySelector('section');
    expect(root?.id).toBe('my-rt');
    expect(root?.getAttribute('style')).toMatch(/max-width:\s*400px/);
    expect(root?.getAttribute('data-testid')).toBe('rt-root');
    expect(root?.getAttribute('aria-describedby')).toBe('hint');
  });

  it('role / tabIndex / lang / dir', () => {
    const { container } = render(<RingToss role="region" tabIndex={-1} lang="zh-TW" dir="ltr" />, {
      wrapper: Wrapper,
    });
    const root = container.querySelector('section');
